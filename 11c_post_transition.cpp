...
{ 
    ...
    for (int i = 0; i < towns_count; i++) { 
        towns[i].name = (char*)malloc(sizeof(char) * (MAX_STRING_LENGTH + 1)); 
        scanf("%6s", towns[i].name); 
        ...
        for (int j = 0; j < towns[i].offices_count; j++) { 
            ...
            for (int k = 0; k < towns[i].offices[j].packages_count; k++) { 
                towns[i].offices[j].packages[k].id = (char*)malloc(sizeof(char) * (MAX_STRING_LENGTH + 1)); 
                scanf("%6s", towns[i].offices[j].packages[k].id); 
    ...
    int queries; 
    scanf("%d", &queries); 
    char town_name[MAX_STRING_LENGTH + 1]; 
